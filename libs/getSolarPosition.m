function [errorCode, spaOutput] = getSolarPosition(spaInput, flagDebug)
%GETSOLARPOSITION Evaluate the position of the sun.
%
% This is a modified version of the external library spa with the input and
% outputs for spa_calculate exposed.
%
% Inputs:
%   - spaInput
%     An spa struct to specify the input values for the NREL's Solar
%     Position Algorithm (SPA), as demonstrated in the external function
%     spa. Please see the *Modifications* section below for more details.
%   - flagDebug
%     Optional. Set this to be true to print out the outputs for debugging.
% Outputs:
%   - errorCode
%     The SPA error code. It will be set to be 0 if no error happens.
%   - spaOutput
%     The updated spa struct with the sun location information.
%
% Yaguang Zhang, Purdue, 12/16/2020
...
% ============= Modifications =============
if (~exist('spaInput', 'var')) || (~isstruct(spaInput))
    disp('Input spa is not a struct! Debug mode is forced to be enabled!');
    flagDebug = true;
    
    % Declare a default SPA structure for debugging with all required
    % fields specified.
    %
    % Note: the results for the original example in spa.m can be verified
    % at
    %     https://midcdmz.nrel.gov/apps/spa.pl?syear=2003&smonth=10&sday=17&eyear=2003&emonth=10&eday=17&step=60&stepunit=1&otype=1&hr=12&min=30&sec=30&latitude=39.742476&longitude=-105.1786&timezone=-7.0&elev=1830.14&press=820&temp=11&dut1=0.0&deltat=67&azmrot=-10&slope=30&refract=0.5667&field=0&field=1&field=3&field=4&field=6&field=7
    % And the results for the default example we use here can be verified
    % at
...
% Define constants needed by SPA.
spa_const;

% Add fields for optional input values for SPA if necessary.
%   - Delta UT1
%     = Universal Time (UT) - Coordinated Universal Time (UTC) [seconds]
if ~isfield(spaInput, 'delta_ut1')
    spaInput.delta_ut1     = 0;
end
%   - DeltaT = Terrestrial Time (TT) - Universal Time (UT) [seconds]
if ~isfield(spaInput, 'delta_t')
    spaInput.delta_t       = 64.797;
end
%   - Annual average local pressure [millibars].
if ~isfield(spaInput, 'pressure')
    spaInput.pressure      = 835;
end
%   - Annual average local temperature [degrees Celsius]
if ~isfield(spaInput, 'temperature')
    spaInput.temperature   = 10;
end
%   - Surface slope (measured from the horizontal plane) [degrees]
if ~isfield(spaInput, 'slope')
    spaInput.slope         = 0;
end
%   - Surface azimuth rotation (measured from south to projection of
%   surface normal on horizontal plane, negative east) [degrees]
if ~isfield(spaInput, 'azm_rotation')
    spaInput.azm_rotation  = 180;
end
%   - Atmospheric refraction at sunrise and sunset [degrees]
if ~isfield(spaInput, 'atmos_refract')
    spaInput.atmos_refract = 0.5667;
end
%   - What to compute:
...
%           SPA_ALL = 3;
if ~isfield(spaInput, 'function')
    spaInput.function      = SPA_ALL;
end

% Call the SPA calculate function and pass the SPA structure
[errorCode, spaOutput] = spa_calculate(spaInput);

if flagDebug
    if (errorCode == 0)  %check for SPA errors
        ...
        fprintf('Sunset:        %2.2d:%2.2d:%2.2d Local Time\n', ...
            floor(spaOutput.sunset), floor(min), floor(sec));
    else
...
% =============== Code from spa.m ===============
%   The original spa.m file can be downloaded from:
%     https://www.mathworks.com/matlabcentral/fileexchange/59903-nrel-s-solar-position-algorithm-spa
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The output of this program should be:
%
%Julian Day:    2452930.312847 L:             2.401826e+01 degrees B:
%-1.011219e-04 degrees R:             0.996542 AU H:             11.105902
%degrees Delta Psi:     -3.998404e-03 degrees Delta Epsilon: 1.666568e-03
%degrees Epsilon:       23.440465 degrees Zenith:        50.111622 degrees
%Azimuth:       194.340241 degrees Incidence:     25.187000 degrees
%Sunrise:       06:12:43 Local Time Sunset:        17:20:19 Local Time
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calculate all SPA parameters and put into structure Note: All inputs
% values (listed in header file) must already be in structure
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

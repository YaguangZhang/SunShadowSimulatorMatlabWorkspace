...
clearvars -except ...
    ...
    pathToSaveSimManDiary PRESETS curPreset ...
...
if exist('flagInitiatedByRoadSimManager', 'var') ...
    ...
    % present and be set to true.
    if flagInitiatedByRoadSimManager == 3
        % In this case, we will use the preset set by curPreset.
        PRESET = curPreset;
    elseif flagInitiatedByRoadSimManager == 2
        % In this case, we will read in the prescan settings from
        % simManConfigs.
        PRESET = 'RoadSimManagerPreScan';
    else
        % In this case, all the simulation settings will be read in from a
        % .mat file generated by the manager.
    ...
    PRESET = 'IN_MotorSpeedway_Oval_SW';
...
if flagInitiatedByRoadSimManager<=2
    % For preset RoadSimManager, we will read in the simulation settings
    % generated by the road simulation manager.
    dirToSimManResultsFolder = fileparts(dirToSaveManDiary);

    % Load simManConfigs and simManState.
    dirToSaveSimManConfigs = fullfile(dirToSimManResultsFolder, ...
        'simManConfigs.mat');
    dirToSaveSimManState = fullfile(dirToSimManResultsFolder, ...
        'simManState.mat');
    load(dirToSaveSimManConfigs); load(dirToSaveSimManState);

    if strcmp(PRESET, 'RoadSimManagerPreScan')
        folderToSaveResults = fullfile(dirToSimManResultsFolder, ...
            simManState.folderToSavePreScanSimResults);
    elseif strcmp(PRESET, 'RoadSimManager')
        folderToSaveResults = fullfile(dirToSimManResultsFolder, ...
...
switch PRESET
    ...
    case 'US41_Seg_Test_Loc_1'
        ...
        simConfigs.GRID_RESOLUTION_IN_M = 3;
    case 'RoadSimManagerPreScan'
        %   - We will read in the simulation settings from simManState.
        simConfigs.LAT_LON_PTS_OF_INTEREST = ...
    ...
    case 'US41_InShadowSeg_DecToMar'
        %   - A test segment on US 41 where a lot of trees are present on
        %   both road sides.
        simConfigs.LAT_LON_BOUNDARY_OF_INTEREST ...
            = [39.53048201143726, -87.40356611643789; ...
            39.53021421863976, -87.40370700784612; ...
            39.53002737873015, -87.40380240225159; ...
            39.53006050804016, -87.4039106077056; ...
            39.53024778252415, -87.40381499171093; ...
            39.53050381117512, -87.40368029008955; ...
            39.53048201143726, -87.40356611643789; ...
            nan, nan; 39.5302992981773, -87.40394696741747; ...
            39.53010241671259, -87.40404748151376; ...
            39.53013554393322, -87.40415568814852; ...
            39.53033270314849, -87.40405503255789; ...
            39.53055342804218, -87.40394015525824; ...
            39.53053164039784, -87.403826043443; ...
            39.5302992981773, -87.40394696741747];
        simConfigs.GRID_RESOLUTION_IN_M = 3;
    case 'US41_InShadowSeg_South_DecToMar'
        %   - The south side of the test segment on US 41 where a lot of
        %   trees are present on both road sides.
        simConfigs.LAT_LON_BOUNDARY_OF_INTEREST ...
            = [39.53048201143726, -87.40356611643789; ...
            39.53021421863976, -87.40370700784612; ...
            39.53002737873015, -87.40380240225159; ...
            ...
            39.53048201143726, -87.40356611643789];
        simConfigs.GRID_RESOLUTION_IN_M = 3;
    case 'US41_InShadowSeg_North_DecToMar'
        %   - The north side of the test segment on US 41 where a lot of
        %   trees are present on both road sides.
        simConfigs.LAT_LON_BOUNDARY_OF_INTEREST ...
            = [39.5302992981773, -87.40394696741747; ...
            39.53010241671259, -87.40404748151376; ...
    ...
    case 'US41_UnderSunSeg_DecToMar'
        ...
        simConfigs.GRID_RESOLUTION_IN_M = 3;
    case {'IN_MotorSpeedway_Oval', 'IN_MotorSpeedway_Oval_SW', ...
            'IN_MotorSpeedway_Oval_6PcSet_NE', ...
        ...
        % The Oval Course (biggest lap) of the Indianapolis Motor Speedway.
        %
        %   - 'IN_MotorSpeedway_Oval'
        %     The largest lap in the circuit.
        %   - 'IN_MotorSpeedway_Oval_SW'
        %     A test for the southwest side of the largest lap.
        %   - 'IN_MotorSpeedway_Oval_6PcSet_[PcLabel]'
        %     The piece label 'PcLabel' can be 'NE', or 'E', 'SE', 'SW',
        %     'W', or 'NW'. Cumulatively, these 6 simulation will cover the
        %     whole largest lap in the circuit. More details about this set
        %     of simulations:
        %       - Date = 2023/03/18;
        %        - Time range = 15:00 to 17:00;
        %       - Time resolution = 15 mins;
        %        - Total time = 2 hrs;
        %       - Space resolution = 2.5 m.
        %
        % Because this route is not avaible in the Indiana Street Center
        % Line dataset, we will need to create the area of interest from a
        % history GPS log file.
        courseInnerPadInM = 30;
        courseOuterPadInM = 10;
        gpsTrackLatLons = readmatrix(fullfile( ...
            ABS_PATH_TO_SHARED_FOLDER, 'Tracks', ...
            'IndianapolisMotorSpeedway', 'OvalCourse.txt'));
        [gpsTrackUtmXs, gpsTrackUtmYs, gpsTrackUtmZones] ...
            = deg2utm(gpsTrackLatLons(:,1), gpsTrackLatLons(:,2));
        assert(all(arrayfun(@(idx) ...
            strcmp(gpsTrackUtmZones(idx, :), ...
            gpsTrackUtmZones(1, :)), 1:size(gpsTrackUtmZones, 1))), ...
            'Not all track points are in the same UTM zone!');
        % Make sure to close the polygon.
        [areaOfIntPolyshapeXs, areaOfIntPolyshapeYs] ...
            = boundary(subtract( ...
            polybuffer(polyshape(gpsTrackUtmXs, gpsTrackUtmYs), ...
            courseOuterPadInM), ...
            polybuffer(polyshape(gpsTrackUtmXs, gpsTrackUtmYs), ...
            -courseInnerPadInM)));

        % For smaller zones, truncate the track polygon accordingly.
        if strcmpi(PRESET, 'IN_MotorSpeedway_Oval_SW')
            NORTH_EAST_BOUND_PT_LAT_LON ...
                = [39.79124397243628, -86.23492779695434];
            [boundXMax, boundYMax, neBoundPtZone] = deg2utm( ...
                NORTH_EAST_BOUND_PT_LAT_LON(1), ...
                NORTH_EAST_BOUND_PT_LAT_LON(2));
            assert(strcmp(neBoundPtZone, gpsTrackUtmZones(1, :)), ...
                ['Not all boundary points ', ...
                'are in the same UTM zone of the track!']);

            boundXMin = min(areaOfIntPolyshapeXs);
            boundYMin = min(areaOfIntPolyshapeYs);
        elseif strcmpi('IN_MotorSpeedway_Oval_6PcSet_', ...
                PRESET(1:min(29, length(PRESET))))
            NORTH_BOUND_PT_LAT_LON = [39.798110, -86.234514];
            SOUTH_BOUND_PT_LAT_LON = [39.791851, -86.234514];

            [nBoundPtX, nBoundPtY, nBoundPtXZone] = deg2utm( ...
                NORTH_BOUND_PT_LAT_LON(1), NORTH_BOUND_PT_LAT_LON(2));
            [sBoundPtX, sBoundPtY, sBoundPtXZone] = deg2utm( ...
                SOUTH_BOUND_PT_LAT_LON(1), SOUTH_BOUND_PT_LAT_LON(2));

            assert(strcmp(nBoundPtXZone, gpsTrackUtmZones(1, :)) ...
                && strcmp(sBoundPtXZone, gpsTrackUtmZones(1, :)), ...
                ['Not all boundary points ', ...
                'are in the same UTM zone of the track!']);

            areaOfIntXMin = min(areaOfIntPolyshapeXs);
            areaOfIntYMin = min(areaOfIntPolyshapeYs);
            areaOfIntXMax = max(areaOfIntPolyshapeXs);
            areaOfIntYMax = max(areaOfIntPolyshapeYs);

            switch PRESET
                case 'IN_MotorSpeedway_Oval_6PcSet_NE'
                    boundXMin= nBoundPtX;
                    boundXMax = areaOfIntXMax;
                    boundYMin = nBoundPtY;
                    boundYMax = areaOfIntYMax;
                case 'IN_MotorSpeedway_Oval_6PcSet_E'
                    boundXMin= nBoundPtX;
                    boundXMax = areaOfIntXMax;
                    boundYMin = sBoundPtY;
                    boundYMax = nBoundPtY;
                case 'IN_MotorSpeedway_Oval_6PcSet_SE'
                    boundXMin= sBoundPtX;
                    boundXMax = areaOfIntXMax;
                    boundYMin = areaOfIntYMin;
                    boundYMax = sBoundPtY;
                case 'IN_MotorSpeedway_Oval_6PcSet_SW'
                    boundXMin= areaOfIntXMin;
                    boundXMax = sBoundPtX;
                    boundYMin = areaOfIntYMin;
                    boundYMax = sBoundPtY;
                case 'IN_MotorSpeedway_Oval_6PcSet_W'
                    boundXMin= areaOfIntXMin;
                    boundXMax = sBoundPtX;
                    boundYMin = sBoundPtY;
                    boundYMax = nBoundPtY;
                case 'IN_MotorSpeedway_Oval_6PcSet_NW'
                    boundXMin= areaOfIntXMin;
                    boundXMax = nBoundPtX;
                    boundYMin = nBoundPtY;
                    boundYMax = areaOfIntYMax;
        ...
        [areaOfIntPolyshapeXs, areaOfIntPolyshapeYs] ...
            = boundary(intersect( ...
            polyshape(areaOfIntPolyshapeXs, areaOfIntPolyshapeYs), ...
            polyshape([boundXMin, boundXMin, ...
            boundXMax, boundXMax, boundXMin], ...
            [boundYMin, boundYMax, boundYMax, boundYMin, boundYMin])));

        [boundOfIntLats, boundOfIntLons] = utm2deg( ...
            areaOfIntPolyshapeXs, ...
            areaOfIntPolyshapeYs, ...
            repmat(gpsTrackUtmZones(1, :), ...
            [length(areaOfIntPolyshapeXs), 1]));

        simConfigs.LAT_LON_BOUNDARY_OF_INTEREST ...
            = [boundOfIntLats, boundOfIntLons];
        simConfigs.GRID_RESOLUTION_IN_M = 2.5;
    otherwise
        error(['Unsupported preset "', PRESET, '"!'])
end
...
if strcmp(PRESET, 'RoadSimManager') ...
        || strcmp(PRESET, 'RoadSimManagerPreScan')
    % For preset RoadSimManager, we will read in the simulation settings
    % generated by the road simulation manager.
...
elseif any(strcmp(decToMarPresetsCell, PRESET))
...
elseif strcmpi('IN_MotorSpeedway_Oval_6PcSet_', ...
                PRESET(1:min(29, length(PRESET))))
    simConfigs.LOCAL_TIME_START = datetime('18-Mar-2023 15:00:00');
    simConfigs.LOCAL_TIME_END = datetime('18-Mar-2023 17:00:00');
    simConfigs.TIME_INTERVAL_IN_M = 15; % In minutes.    
else
